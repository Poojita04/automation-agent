...
def a8_credit_card_image():
    """Generate a credit card image at D:\TDS\automation-agent\data\credit_card.png"""
    ...
    # Correct the path
    correct_path = os.path.join("D:\\TDS\\automation-agent\\data", "credit_card.png")
    print("Saving image at:", correct_path)
...
def get_comments(email):
    """Generate and save 100 fake comments to /data/comments.txt"""
    fake = Faker()
    fake.seed_instance(hash(f"{email}:a9"))  # Seed for consistency

    comments = [fake.paragraph() for _ in range(100)]  # Generate 100 random comments
...
def load_comments():
    with open(os.path.join(config["root"], "data", "comments.txt"), "r", encoding="utf-8") as file:
...
def find_most_similar_comments(comments):
    model = SentenceTransformer("all-MiniLM-L6-v2")  # Efficient small model for embeddings
    embeddings = model.encode(comments)  # Convert each comment to an embedding

    # Compute cosine similarity
    similarity_matrix = cosine_similarity(embeddings)

    # Avoid self-comparisons by setting diagonal to a low value
